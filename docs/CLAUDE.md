...
This file lives in `docs/` alongside `ARCHITECTURE.md` and `DATA-MODEL.md`. **Note:**
Claude Code only auto-loads project instructions from `.claude/CLAUDE.md` at session
start, so a one-line stub lives there pointing back to this file — edit standards here,
not the stub. Keep it up to date as decisions change — it's the source of truth for how
...
docs/                   # ARCHITECTURE.md, DATA-MODEL.md, CLAUDE.md (see below)
```
